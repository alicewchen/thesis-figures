...
model <- lm(mean_PE_A ~ Latitude*Temperature*Light*Strain*Timepoint, data = df)
aov<- aov(model)
summary(aov) 
...
model <- df %>% 
  #filter (Timepoint ==24) %>% #If the lipid response to light occurs at 24, analyze time point = 24 only
  filter(Strain != "3064") %>%
...
sink(file = "../analysis/all_interactions_no_outlier_no_strain.txt")
model <- lm(mean_PE_A ~ Latitude + Temperature + Timepoint + Light + 
              Latitude*Light+ 
              Latitude*Timepoint+ 
              Latitude*Temperature + 
              Temperature*Timepoint +  
...
summary(aov)
sink(file = NULL)
pdf("../analysis/diagnostic_plots_all_interactions_no_outlier_no_strain.pdf", height = 6, width = 6)
plot(model)
qqnorm(resid(model))
qqline(resid(model))
hist(resid(model))
...
#------------------------------------------------------------------
# Linear model including all two-way interactions (with outlier)
# Strain as random effect ####
sink(file = "../analysis/all_interactions_with_outlier_strain_as_random.txt", type = c("output", "message"))
model <- lmer(mean_PE_A ~ 
                ...
                Timepoint*Light+
                (1|Strain), data = df)
aov<- Anova(model,type="III", test = "F")
summary(model)
aov
sink(file = NULL)
pdf("../analysis/diagnostic_plots_all_interactions_with_outlier_strain_as_random.pdf", height = 6, width = 6)
plot(model)
qqnorm(resid(model))
qqline(resid(model))
hist(resid(model))
...
#------------------------------------------------------------------
# Linear model including all two-way interactions (with outlier)
# Strain nested in latitude as random effect ####
sink(file = "../analysis/all_interactions_with_outlier_strain_nested_in_latitude.txt", type = c("output", "message"))
model <- lmer(mean_PE_A ~ Latitude+Light+Timepoint+Temperature+
                Latitude*Light+ 
                Latitude*Timepoint+ 
                Latitude*Temperature + 
                Temperature*Timepoint +  
                Temperature*Light+ 
                Timepoint*Light+ (1+Latitude|Strain)+(1|Strain), data = df)
aov<- Anova(model, test="F", type="III")
summary(model)
aov
sink(file = NULL)
pdf("../analysis/diagnostic_plots_all_interactions_with_outlier_strain_nested_in_latitude.pdf", height = 6, width = 6)
plot(model)
qqnorm(resid(model))
qqline(resid(model))
hist(resid(model))
...
model <- lmer(mean_PE_A ~ 
                ...
                Temperature*Timepoint +  
                Temperature*Light+ 
                Timepoint*Light+ (1+Latitude|Strain)+
                (1|Strain), data = clean)
aov<- anova(model)
...
# Linear model including all two-way interactions (with outlier); 
# no random effects####
model <- lm(mean_PE_A ~ Latitude + Temperature + Timepoint + Light + Strain +
                Latitude*Light+ 
                Latitude*Timepoint+ 
                Latitude*Temperature + 
                Temperature*Timepoint +  
                Temperature*Light+ 
                Timepoint*Light+
              Strain*Latitude, data = df)
aov<- aov(model)
sink(file = "../analysis/all_interactions_no_outlier_strain_as_main.txt")
summary(model)
summary(aov)
sink(file = NULL)
pdf("../analysis/diagnostic_plots_all_interactions_no_outlier_strain_as_main.pdf", height = 6, width = 6)
plot(model)
qqnorm(resid(model))
qqline(resid(model))
hist(resid(model))
...
#------------------------------------------------------------------
# Linear model including all two-way interactions (with outlier); 
# no random effects; no strain interaction####
sink(file = "../analysis/all_interactions_no_outlier_strain_as_main_not_interaction.txt")
model <- lm(mean_PE_A ~ Latitude + Temperature + Timepoint + Light + Strain +
              Latitude*Light+ 
              Latitude*Timepoint+ 
              Latitude*Temperature + 
...
summary(aov)
sink(file = NULL)
pdf("../analysis/diagnostic_plots_all_interactions_no_outlier_strain_as_main_not_interaction.pdf", height = 6, width = 6)
plot(model)
qqnorm(resid(model))
qqline(resid(model))
hist(resid(model))
